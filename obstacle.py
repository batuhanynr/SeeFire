...
class ObstacleAvoidance:
    ...
    def _choose_open_side(self) -> str:
        ...
        logger.info(
            "[OBSTACLE] Yan açıklık — Sol: %.1f cm | Sağ: %.1f cm | Ön: %.1f cm",
            left, right, reading.front_cm,
        )

        # Geçersiz/menzil-aşımı okuma (0 veya negatif) → "açık" varsay.
        ...
        # Benzer açıklık → kamera ipucu, yoksa marjinal olarak açık taraf.
    ...
    def _side_pass(self, sector_id: int, direction: str,
        ...
        while True:
            ...
            if clearance > clear_threshold:
                logger.info(
                    "[BYPASS-YAN] Engel geçildi (Açıklık: %.1f cm > Eşik: %.1f cm). "
                    "Gövde tam temizlenene kadar ekstra adımlar atılıyor...",
                    clearance, clear_threshold,
                )
                # Sensör gövdenin önünde — sensör temizlendiğinde arka kısım
                # hâlâ engel hizasında olabilir. Dönüşte arkayı engele çarpmamak
                # için gövde boyu kadar ekstra yanal ilerleme gerekli.
                extra_steps = 3  # 3 × 5cm = 15cm (gövde uzunluğu payı)
                for step_idx in range(extra_steps):
                    logger.info(
                        "[BYPASS-YAN] Ekstra güvenlik adımı %d/%d...",
                        step_idx + 1, extra_steps,
                    )
